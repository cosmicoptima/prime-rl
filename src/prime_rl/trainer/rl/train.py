    ...
    while True:
        ...
        for micro_step, micro_batch in enumerate(micro_batches):
            ...
            if config.model.liger_kernel and config.loss.type == "grpo":
                ...
                shifted_logits = shift_logits(logits)
                shifted_logits = shifted_logits / temperature
                fresh_logprobs = selective_log_softmax(shifted_logits, input_ids)
                ...
                # Compute is_clipped for logging (convert to float for stats)
                is_clipped = (importance_ratio > config.loss.clip_ratio).float()
